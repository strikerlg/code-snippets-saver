...
$db = DB::getInstance();
$searchkey = $db->filter($_GET['q']);

if ($authenticated == false) {
    $query = "SELECT * FROM ".DBTABLE_SNIPPETS." WHERE visibility='public' AND name LIKE '%{$searchkey}%' ORDER BY id DESC LIMIT 20";
    $total_found = $db->num_rows($query);
    if ($total_found > 0) {
        $results = $db->get_results($query);
        $html_content = '<div class="list-group snippet-list">';
        foreach ($results as $row) {
            $html_content .= '<a href="view_snippet.php?id='.$row['id'].'" class="list-group-item"><span class="snippet-title">'.$row['name'].'</span> <span class="label '.get_category_color($row['category_tag']).'">'.get_category_title($row['category_tag']).'</span> &middot; <span class="by-creator"> by <strong>'.get_username($row['user_id']).'</strong></span> &middot; <span class="created-date">'.get_timeago($row['updated_time']).'</span></a>';
        }
        $html_content .= '</div>';
    } else {
    ...
    // authenticated == true
    $query = "SELECT * FROM ".DBTABLE_SNIPPETS." WHERE name LIKE '%{$searchkey}%' ORDER BY id DESC LIMIT 20";
    $total_found = $db->num_rows($query);
    if ($total_found > 0) {
        $results = $db->get_results($query);
        $html_content = '<div class="list-group snippet-list">';
        foreach ($results as $row) {
            if ($row['visibility'] == 'private' && $row['user_id'] == $user_loggedin_id) {
                $html_content .= '<a href="view_snippet.php?id='.$row['id'].'" class="list-group-item"><span class="snippet-title">'.$row['name'].'</span> <span class="label '.get_category_color($row['category_tag']).'">'.get_category_title($row['category_tag']).'</span> &middot; <span class="by-creator"> by <strong>'.get_username($row['user_id']).'</strong></span> &middot; <span class="created-date">'.get_timeago($row['updated_time']).'</span><br><span class="private-snippet"><i class="fa fa-info-circle"></i> This is your private snippet. Only you can see this!</span></a>';
    ...
    header('Location: 404.php?error=empty_parameter');
    exit;
}
...
<body>
            ...
            <div class="col-md-9" id="rightSide">
                <div class="panel panel-default box">
                    <div class="panel-heading" id="contentHeading">
                        <i class="fa fa-list-alt"></i> Search for <strong><?php echo $searchkey; ?></strong>
                    </div>
    ...
    <script>
      ...
      $(".snippet-title").highlight("<?php echo $searchkey; ?>");
    </script>
